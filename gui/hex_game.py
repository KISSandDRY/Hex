...
class HexGameManager:
    ...
    def handle_input(self, events):
        ...
        is_ai_turn = (self.mode == GameMode.PVAI and self.turn != self.human_player)
    ...
    def _draw_turn_label(self):
        ...
        color = TEXT_COLOR if self.thinking else (P1_COLOR if self.turn == PLAYER_1 else P2_COLOR)
        
        if self.mode == GameMode.PVAI:
            txt += " (You)" if self.turn == self.human_player else " (AI)"

        self._draw_text(txt, color, (140, 25), FONT_SIZE)

    def _draw_winner_text(self):
        winner_name = "Red" if self.winner == PLAYER_1 else "Blue"
        msg = f"{winner_name} wins!"
        self._draw_centered_boxed_text(msg, WIN_PATH_COLOR, 50)
    ...
    def _calc_offset(self):
        ...
        self.render_cfg["offset_x"] = int((SCREEN_WIDTH - board_px_w) // 2)
        self.render_cfg["offset_y"] = (SCREEN_HEIGHT - board_px_h) // 2 + 100
